...
private:
    ...
    struct SceneData {
        ...
        static_assert(sizeof(DirectionalLight) % 16 == 0, "Directional light data must be a multiple of 16 bytes in width");
...
public:
    ...
    void set_depth_testing(bool p_value) { use_depth_test = p_value; }
    ...
    virtual void item_set_texture_rect(RID p_item, RID p_texture, const Vector2 &p_position, const Vector2 &p_size) override;
    ...
    virtual void texture_set_from_data(RID p_texture, const u8 *p_data, int p_width, int p_height, TextureFormat p_format, TextureMask p_mask) override;
