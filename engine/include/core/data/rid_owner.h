...
class RIDAllocator : public RIDAllocatorBase {
    ...
    // Items are the base of the RID allocations. They contain the value to hold, as well as a validator to let us know if the position is free or not.
    struct Item {
        T data;
        u32 validator;
    };
...
public:
    ...
    FORCE_INLINE T *get_or_null(const RID &p_rid, bool p_initialize = false) {
        ...
        if (p_initialize) {
            ...
            ERR_FAIL_COND_MSG_R((i.validator & 0x7fffffff) != validator, "Attempted to initialize the wrong RID.", nullptr);
    ...
    FORCE_INLINE void free(const RID &p_rid) {
        ...
        u32 validator = u32(id >> 32);
        ERR_FAIL_COND_MSG(items[chunk_id][chunk_idx].validator & 0x80000000, "Attempted to free an uninitialized RID.");
        ...
        items_allocated--;
        free_list[items_allocated / items_per_chunk][items_allocated % items_per_chunk] = index; // Make the now-freed item the next spot on the free list
    }
...
/**
 * An RID Owner stores the data associated with the RID as well as a validator code that associates to the bitwise value of the RID.
 * Every RID stores its bit value in the 1st half of the number (reading lowest to highest from right to left)
 * An RID "item" is a struct of the actual item itself and its validation code (a 32-bit uint which is equal to the RID shifted 32 bits to the right)
 * Since these allocators need to be fast if dealing with many different RIDs, we want to be more spare with memory allocations.
 * "Chunks" are regions of memory that can allow for multiple different RID positions to be set up. 
 * 
 */
