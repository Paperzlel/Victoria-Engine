...
/**
 * @brief Structure that represents the current keyboard layout. Each key is represented as a boolean for whether it is pressed down or not.
 */
struct Keyboard {
public:
    bool keys[256];
...
/**
 * @brief Structure that represents the current mouse wheel layout. Contains all the mouse buttons, as well as the scroll amount (-1 or 1) and the position of 
 * the cursor as a Vector2i.
 */
struct Mouse {
public:
    bool buttons[3];
...
public:
     ...
     */
    u8 get_mouse_scroll() const { return current_mouse.scroll; }

    // Mouse movement commands

    /**
     * @brief Gets the current position of the mouse in absolute screen coordinates, with (0, 0) being in the top-leftmost corner in the Windows implementation.
     */
    Vector2i get_mouse_screen_position() const { return current_mouse.position; }
