 ...
 */
VAPI void _err_print_err(const char *file, const char *function, int line, const char *err_msg, const char *msg = "", ErrorType type = ERROR_ERR);
 ...
 * @param type The type of error message that is being sent
 */
VAPI void _err_print_index_err(const char *file, const char *function, int line, int m_index, int m_size, const char *err_msg, const char *msg = "", ErrorType type = ERROR_ERR);
...
#ifdef _MSC_VER
// Defines a way to add a breakpoint into the code, without a user needing to create one in their own IDE. Should only be used whenever a crash-worthy or very severe error has come up. 
...
/**
 * @brief Generic error method. Notifies the user and returns. Should only be used where there is no suitable message to give the user.
 */
#define ERR_FAIL()                                                                  \
...
/**
 * @brief Generic error method. Notifies the user and returns a given value, as well as printing a user-created message to the console.
...
#define ERR_OUT_OF_BOUNDS_R(m_index, m_size, m_retval)                                                                                              \
    if (m_index < 0 || m_index > m_size) {                                                                                                          \
        _err_print_index_err(__FILE__, FUNCTION_STR, __LINE__, m_index, m_size, "Index given was out of bounds. Returning " _STR(m_retval) ".");    \
        return m_retval;                                                                                                                            \
    }                                                                                                                                               \
...
#define ERR_OUT_OF_BOUNDS_MSG_R(m_index, m_size, m_msg, m_retval)                                                                                           \
    if (m_index < 0 || m_index > m_size) {                                                                                                                  \
        _err_print_index_err(__FILE__, FUNCTION_STR, __LINE__, m_index, m_size, "Index given was out of bounds. Returning " _STR(m_retval) ".", m_msg);     \
        return m_retval;                                                                                                                                    \
    }                                                                                                                                                       \
...
#define ERR_FAIL_COND_R(m_cond, m_retval)                                                                                               \
    if (cond(m_cond)) {                                                                                                                 \
        _err_print_err(__FILE__, FUNCTION_STR, __LINE__, "Condition \"" _STR(m_cond) "\" was true. Returning " _STR(m_retval) ".");     \
        return m_retval;                                                                                                                \
    }                                                                                                                                   \
...
#define ERR_FAIL_COND_MSG_R(m_cond, m_msg, m_retval)                                                                                        \
    if (cond(m_cond)) {                                                                                                                     \
        _err_print_err(__FILE__, FUNCTION_STR, __LINE__, "Condition \"" _STR(m_cond) "\" was true. Returning " _STR(m_retval) ".", m_msg);  \
        return m_retval;                                                                                                                    \
    }                                                                                                                                       \
...
#define ERR_COND_FATAL(m_cond)                                                                                                      \
    if (cond(m_cond)) {                                                                                                             \
        _err_print_err(__FILE__, FUNCTION_STR, __LINE__, "Condition \"" _STR(m_cond) "\" is true, a fatal error has occured.");     \
        DEBUG_BREAK();                                                                                                              \
    }                                                                                                                               \
...
#define ERR_COND_NULL_R(m_item, m_retval)                                                                                           \
    if (cond(m_item == nullptr)) {                                                                                                  \
        _err_print_err(__FILE__, FUNCTION_STR, __LINE__, "Item \"" _STR(m_item) "\" is null. Returning " _STR(m_retval) ".");       \
        return m_retval;                                                                                                            \
    }                                                                                                                               \
...
#define ERR_COND_NULL_MSG_R(m_item, m_msg, m_retval)                                                                                    \
    if (cond(m_item == nullptr)) {                                                                                                      \
        _err_print_err(__FILE__, FUNCTION_STR, __LINE__, "Item \"" _STR(m_item) "\" is null. Returning " _STR(m_retval) ".", m_msg);    \
        return m_retval;                                                                                                                \
    }                                                                                                                                   \

/**
 * @brief Throws an error whenever the given item is null. This macro does not return early, but instead calls to `DEBUG_BREAK()` to add a breakpoint at the 
 * current function. Should only ever be used in a situation where the error the program currently faces cannot allow for the application to continue, and needs 
 * to be inspected carefully.
 * @param m_item The given item to assess as null
 */
#define ERR_COND_NULL_FATAL(m_item)                                                                                         \
    if (cond(m_item == nullptr)) {                                                                                          \
        _err_print_err(__FILE__, FUNCTION_STR, __LINE__, "Item\"" _STR(m_item) "\" is null, a fatal error has occured.");   \
        DEBUG_BREAK();                                                                                                      \
    }                                                                                                                       \
