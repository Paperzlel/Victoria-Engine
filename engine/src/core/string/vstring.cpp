...
/**
 * @brief Finds the first occurence of a given string in another string. In this instance, the string is case-sensitive and will not work if there
 * is a difference in capitalisation.
 * @param p_what The string to look for in the larger string
 * @returns The point in the string where the first character of the string given appears if successful, and `-1` if the string could not be found.
 */
...
/**
 * @brief Obtains the number of instances of a given string within a broader string. This version is case-sensitive and will not work if there
 * is a difference in capitalisation.
 */
...
/**
 * @brief Returns a copy of the current string that is the first n characters in the given string (left to right). If n is less than 0, then 
 * n characters are removed from the end of the string. 
...
/**
 * @brief Returns a copy of the current string that is the last n characters in the given string (left to right). If n is less than 0, then n
 * characters are removed from the start of the string.
 * @param p_count The length of the new string
 * @returns The string as described above
 */
...
/**
 * @brief Method to separate one string into several smaller ones, with a delimiter as the indicator for how one wants to split the string.
...
Vector<String> String::split(const String &delimiter) const {
    ...
    // At the end of the process, we need to add the other string in. - 1 is for the offset to prevent including 2 null terminators
...
/**
 * @brief Takes a string and turns it into an integer. This applies universally to a string, so any non-numerical characters will still be read as a 
 * number, which can confuse the function and give a non-intended result.
...
/**
 * @brief Method to copy in a C string to the class from another location. Assumes that we know the string is not null, and that its length is greater than zero, and so forth.
