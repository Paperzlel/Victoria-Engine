...
/**
 * @brief Checks if the given quaternion is an identity quaternion, which is when all the axes are set to zero and the angle is set to 1.
...
Vector3 Quaternion::get_axis() const {
    ...
    if (is_identity() || angle == 0) {
        return Vector3::zero(); // acos of 1 is zero, and sin of 0 is zero so we want to avoid a divide-by-zero error here
    }
...
/**
 * @brief Converts the current quaternion into a rotation matrix, which can then be used to construct a model matrix or something else along those lines.
...
/**
 * @brief Creates and returns a quaternion that was constructed from a set of Euler angles. Expects the rotational order for said Euler angles to be YXZ.
