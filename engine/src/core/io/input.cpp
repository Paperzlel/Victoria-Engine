...
/**
 * @brief Checks if the given key has just began to be held down by the user. This event should only occur once, when the user begins to hold down the key.
 */
...
/**
 * @brief Checks if the given key has just stopped being held down by the user. This event should only occur once, when the user begins to hold down the key.
 */
...
/**
 * @brief Checks if the given mouse button has just begun to be pressed down by the user. This event should only occur once, when the user begins to hold down the button.
 */
...
/**
 * @brief Checks if the given mouse button is not being held down by the user. This event should only occur once, when the user begins to hold down the button.
 */
...
/**
 * @brief Returns the relative (the change in x/y position) position of the cursor on the screen. The general origin for the given cursor is calculated as the centre of
 * the screen, and any movements are then limited to whether the cursor is captured or not.
 */
...
Vector2 Input::get_vector(Key p_left, Key p_right, Key p_up, Key p_down) {
    float x = is_key_pressed(p_left) && is_key_pressed(p_right) ? 0 : (is_key_pressed(p_left) ? -1 : (is_key_pressed(p_right) ? 1 : 0));
    float y = is_key_pressed(p_down) && is_key_pressed(p_up) ? 0 : (is_key_pressed(p_down) ? -1 : (is_key_pressed(p_up) ? 1 : 0));
    Vector2 ret = {x, y};
    return ret;
}
...
void Input::parse_input_event(const Ref<InputEvent> &p_event) {
    ...
    if (key_event.is_valid()) {
        ...
        // Send off additional calls for if the left/right control keys are pressed, since their modifiers will also affect the default keys.
...
/**
 * @brief Updates the keyboards - it pushes the current keyboard and mouse from the current to the previous, so that when the key processing has occured any events
 * that rely on a difference in keypresses can be detected.
 */
void Input::update() {
