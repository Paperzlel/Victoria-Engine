...
/**
 * @brief Opens the file specified by the given path and uses a given kind of read/write operation. Currently no support for reading AND writing.
 * @param p_path The path to the given file to load. Can be absolute, but relative paths are relative to the place of the `victoria` binary on your PC.
...
/**
 * @brief Obtains the current position of the file handle within the file. File position is read from 0, where 0 is the space before the first character.
 */
...
/**
 * @brief Finds the length (number of characters between 0 and the null terminator) of the currently used file. Returns to the origin of the file, rather than the 
...
/**
 * @brief Reads all of the text within a file as a string and returns that to the user, as well as returning the file position back to the start.
 */
...
/**
 * @brief Gets the next line of the current file. A line is specified as the point between the start of the file and its first newline character, between 
 * two newline characters, or between one newline character and the end of the file. 
...
String FileSystem::get_line() {
    ...
    String ret;
    ret.resize(offset); // offset naturally adds 1 to itself, so string resizing doesn't need to account for the null terminator
    fseek(handle->f, handle->pos, SEEK_SET);
