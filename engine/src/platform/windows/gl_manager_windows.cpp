...
typedef int(__cdecl *PFN_NvAPI_DRS_GetApplicationInfo)(NVDRSSessionHandle,
													   NvDRSProfileHandle,
													   NvAPI_UnicodeString,
													   NVDRS_APPLICATION *);
...
/**
 * @brief Creates an NVIDIA profile to disable a couple of undesired settings, namely vsync being enabled globally
 * (which we want to have control over) and disabling threaded optimisation, which causes stuttering on occasion, which
 * we also don't want.
 */
...
#	endif // _WIN64
	...
	PFN_NvAPI_DRS_DestroySession NvAPI_DRS_DestroySession =
		(PFN_NvAPI_DRS_DestroySession)NvAPI_QueryInterface(0x0DAD9CFF8);
	...
	PFN_NvAPI_DRS_FindProfileByName NvAPI_DRS_FindProfileByName =
		(PFN_NvAPI_DRS_FindProfileByName)NvAPI_QueryInterface(0x7E4A9A0B);
	PFN_NvAPI_DRS_CreateProfile NvAPI_DRS_CreateProfile =
		(PFN_NvAPI_DRS_CreateProfile)NvAPI_QueryInterface(0x0CC176068);
	PFN_NvAPI_DRS_GetApplicationInfo NvAPI_DRS_GetApplicationInfo =
		(PFN_NvAPI_DRS_GetApplicationInfo)NvAPI_QueryInterface(0x0ED1F8C69);
	PFN_NvAPI_DRS_CreateApplication NvAPI_DRS_CreateApplication =
		(PFN_NvAPI_DRS_CreateApplication)NvAPI_QueryInterface(0x4347A9DE);
	...
	if (success != 0) {
		...
		if (!nvapi_check_error("NVAPI: Error creating profile",
							   NvAPI_DRS_CreateProfile(s_handle, &profile_info, &p_handle))) {
			NvAPI_DRS_DestroySession(s_handle);
			NvAPI_Unload();
			return;
		}
	...
	if (success != 0) {
		...
		if (!nvapi_check_error("NVAPI: Error creating application information",
							   NvAPI_DRS_CreateApplication(s_handle, p_handle, &app))) {
			NvAPI_DRS_DestroySession(s_handle);
			NvAPI_Unload();
			return;
		}
	...
	if (!nvapi_check_error("NVAPI: Couldn't save the setting correctly",
						   NvAPI_DRS_SetSetting(s_handle, p_handle, &thread_setting))) {
		NvAPI_DRS_DestroySession(s_handle);
		NvAPI_Unload();
		return;
	}
	...
	if (!nvapi_check_error("NVAPI: Couldn't set the setting correctly",
						   NvAPI_DRS_SetSetting(s_handle, p_handle, &vsync_setting))) {
		NvAPI_DRS_DestroySession(s_handle);
		NvAPI_Unload();
		return;
	}
...
Error GLManagerWindows::_create_context(GLWindow &win) {
	...
	int attribs[] = {
		...
		6, // Want a 4.6 context (will fallback to 3.3 eventually)
		WGL_CONTEXT_PROFILE_MASK_ARB,
		WGL_CONTEXT_CORE_PROFILE_BIT_ARB,
		WGL_CONTEXT_FLAGS_ARB,
		WGL_CONTEXT_FORWARD_COMPATIBLE_BIT_ARB, // Forward compatible so that it is only using up-to-date functionality
		0};

	// NOTE: These two function pointers COULD be in the `find_wgl_extensions` function, however due to their necessity
	// now, it's better to pre-load them beforehand.
...
/**
 * @brief Method to initialize the OpenGL manager. When called, it gets an OpenGL version, and queries for the WGL
 * extensions available at a given time. It also calls to load GLAD at the same time, in order for the function
 * pointers used by OpenGL to work
