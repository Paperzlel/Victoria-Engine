 ...
 */
void OSWindows::print_error(const char *file,
							const char *function,
							int line,
							const char *err_msg,
...
/**
 * @brief Method that contains the main loop of the application. Main's version of this runs somewhat differently, in
 * that it runs an iteration of the system's loop, and is the point by which the application can actually terminate.
 */
void OSWindows::run() {
	if (!main_loop) {
		return;
...
/**
 * @brief Class constructor for the OS class. Initializes the instance, and checks to see if we can use virtual
 * terminal processing, or in other words, if we can use Unix-like coloured text in the terminal.
