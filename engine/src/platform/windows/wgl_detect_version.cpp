...
/**
 * @brief Method to obtain the current OpenGL version from a temporary context. Currently uses a vector, but should use a Dictionary in the future if needed.
 * @return A vector containing all of the information we want - first index is the GL version, second is the card manufacturer, third is driver version. If the function failed, it will return an empty dictionary, and contains no data. Assume this is a failure of the program.
 */
                                ...
                                if (new_hRC) {
                                    if (wglMakeCurrent(hDC, new_hRC)) {
                                        ...
                                        if (victoria_wglGetString) {
                                            ...
                                            if (version) {
                                                const String &vendor = String(victoria_wglGetString(GL_VENDOR)).remove_suffix(" Corporation");
                                                const String &device = String(victoria_wglGetString(GL_RENDERER)).remove_suffix("/PCIe/SSE2");
                                                ...
                                                const String &version = vformat("%d.%d.%d", version_major, version_minor, version_patch);
