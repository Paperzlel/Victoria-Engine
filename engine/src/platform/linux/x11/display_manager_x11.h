...
public:
    ...
    /**
     * @brief Returns a pointer to the current display server being used. Due to our goal of self-containment, this function should not be usable
     * outside of the X11 interface to avoid compiler errors.
     * @returns A pointer to the current X11 display being used.
     */
    Display *get_display() const { return display; }
    /**
     * @brief Gets a pointer to the current screen the X11 interface is rendering to. Should be used only within any X11-related code to prevent
     * any compiler issues.
