...
Error GLManagerX11::_create_context(GLWindow &win) {
	...
	GLXContext context = glXCreateContextAttribsARB(display, fb_config, nullptr, true, context_attribs);
	XSync(display, false);
	if (!context) {
	...
	victoria_glXSwapIntervalEXT = (PFNGLXSWAPINTERVALEXT)glXGetProcAddress((const GLubyte *)"glXSwapIntervalEXT");
	if (victoria_glXSwapIntervalEXT == nullptr) {
...
/**
 * @brief Method to obtain the "visual info" of a given screen, which is used in window creation to determine if a
 * certain type of OpenGL context can be created.
...
XVisualInfo *GLManagerX11::get_visual_info() {
	...
	int glx_attribs[] = {GLX_X_RENDERABLE,
						 true,
						 GLX_DRAWABLE_TYPE,
						 GLX_WINDOW_BIT,
						 GLX_RENDER_TYPE,
	...
	if (screen_id != visual->screen) {
		OS::get_singleton()->print("screen_id (%i) does not match that of visual->screen (%i)",
								   screen_id,
								   visual->screen);
		XCloseDisplay(display);
		return nullptr;
 ...
 */
uint8_t GLManagerX11::create_window(Window *win) {
	GLWindow *n_window = vnew(GLWindow);
	n_window->id = 0;
	n_window->win = win;
 ...
 */
void GLManagerX11::destroy_window(uint8_t p_id) {
	if (!_active_window) {
		return;
	}
 ...
 * @returns `OK` if all went well, and `ERR_NULL` if the `DisplayManagerX11` could not be found (in which case,
 * something has gone seriously wrong)
 */
Error GLManagerX11::initialize() {
	DisplayManagerX11 *ds = static_cast<DisplayManagerX11 *>(DisplayManagerX11::get_singleton());
	ERR_COND_NULL_R(ds, ERR_NULL);
...
/**
 * @brief The finalizing step of the OpenGL manager. Releases all resources that are no longer needed, as well as the
 * active window.
 */
...
/**
 * @brief Swaps around the two different framebuffers to present the current frame. Is only available if there is
 * double framebuffer support, which for our sake we will assume that you do have access to that.
 */
