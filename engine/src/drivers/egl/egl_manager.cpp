...
Error EGLManager::_create_context(EGL_Display &p_display) {
    ...
    p_display.context = eglCreateContext(p_display.egl_display, p_display.config, EGL_NO_CONTEXT, (ctx_attribs.size() > 0) ? ctx_attribs.ptr() : nullptr);
    ERR_FAIL_COND_MSG_R(p_display.context == EGL_NO_CONTEXT, vformat("Can't create an EGL context, error %d.", eglGetError()), ERR_CANT_CREATE);
...
int EGLManager::_get_display_id(void *p_native) {
    ...
    if (GLAD_EGL_VERSION_1_5) {
        display.egl_display = eglGetPlatformDisplay(platform, display.native_display, (attrib_list.size() > 0) ? attrib_list.ptr() : nullptr);
...
u8 EGLManager::create_window(void *p_native_display, void *p_native_window) {
    ...
    if (GLAD_EGL_VERSION_1_5) {
        ...
        win->surface = eglCreatePlatformWindowSurfaceEXT(display.egl_display, display.config, p_native_window, nullptr);
#endif
    } else {
...
Error EGLManager::initialize(void *p_platform_display) {
    // Should really make a null connection, but it was crashing on libLLVM.so.20.1 for some reason.
    // Genuinely no idea how this isn't working. Checked compiling alone, works, Checked Godot with debug symbols, works. Here? Doesn't work!
