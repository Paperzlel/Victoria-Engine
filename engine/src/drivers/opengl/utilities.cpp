...
Error Utilities::check_pipeline_errors(u32 p_what, GLStatusType p_compile_type) {
    ...
    switch (p_compile_type) {
        ...
        default: {
            OS::get_singleton()->print_error(__FILE__, FUNCTION_STR, __LINE__, vformat("OpenGL: Shader pipeline error handler was passed an invalid error of %i.", p_compile_type), "", ERROR_SHADER);
...
void Utilities::debug_message_callback(GLenum p_source, GLenum p_type, GLenum p_id, GLenum p_severity, GLsizei p_length, 
                                const GLchar *p_message, const void *p_user_param) {
    ...
    if (severity > 1) {
        // Non-error
        error_string = vformat("OpenGL: %s gave a %s %s: \n%s", source_location_strings[source_id], error_type_strings[error_type], severity_strings[severity], p_message);
    } else {
        // Error
        error_string = vformat("OpenGL: %s gave a %s %s error: \n%s", source_location_strings[source_id], severity_strings[severity], error_type_strings[error_type], p_message);
    }
