...
/**
 * @brief Converts a `Mat4` into an array of floats to upload to the GPU. Since double-precision isn't supported for positions in GLSL, we have to manually
 * convert the matrix from doubles into floats, which is done by this function.
...
Error RenderingServerGL::initialize() {
    ...
    if (err != OK) {
        OS::get_singleton()->print_error( __FILE__, FUNCTION_STR, __LINE__, "OpenGL: Scene shader failed to compile.", error_messages[err]);
    }

    err = shaders.canvas_shader._init();
    if (err != OK) {
        OS::get_singleton()->print_error(__FILE__, FUNCTION_STR, __LINE__, "OpenGL: Canvas shader failed to compile.", error_messages[err]);
    }
...
void RenderingServerGL::finalize() {
    ...
    utils->free_buffer(GL_UNIFORM_BUFFER, sizeof(SceneData::DirectionalLight) * 32, &scene_data.directional_light_buffer);
    ...
    utils->free_buffer(GL_ARRAY_BUFFER, sizeof(CanvasInstanceData) * MAX_INSTANCE_DATA_COUNT, &canvas_data.canvas_instance_data_buffer);
...
/**
 * @brief Sorts the active viewports from most nested to the root window. Since each framebuffer can reference a viewport's texture,
 * the most nested viewport must be drawn first to give the correct texture to the other viewports.
...
void RenderingServerGL::_render_internal(RenderData *r_data) {
    ...
    // - Renders Viewport 1 first (get scene list --> render, get item list (empty) --> render, render viewport texture)
...
void RenderingServerGL::_render_scene(RenderData *r_data, Viewport *p_viewport, Camera *p_camera) {
    // Scene pass
    {
                ...
                case INSTANCE_TYPE_LIGHT: {
                    ...
                    switch (light->type) {
                        case TYPE_DIRECTIONAL_LIGHT: {
                            ...
                            SceneData::DirectionalLight &p = scene_data.directional_lights[directional_light_count - 1];
                            ...
                            Vector3 dir;/* = (inv_cam_view.basis *
                                    Mat4::translation((inst->transform.basis * Mat4::translation(Vector3(0, 0, 1))).get_translation()))
                                    .get_translation()
                                    .normalized(); */
                        ...
                        case TYPE_SPOT_LIGHT: {
                            ...
                            Vector3 pos = (inv_cam_view.basis * Mat4::translation(inst->transform.position)).get_translation();
        ...
        glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(SceneData::PointLight) * point_light_count, scene_data.point_lights);
        ...
        glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(SceneData::DirectionalLight) * directional_light_count, scene_data.directional_lights);
...
void RenderingServerGL::_render_canvas(RenderData *r_data, Viewport *p_viewport, Canvas *p_canvas) {
    ...
    // Canvas shader pass
    {
        ...
        for (const Item *i : p_canvas->child_items) {
            ...
            if (b->material != canvas_data.batches[canvas_data.current_batch].material || i->ysort != canvas_data.batches[canvas_data.current_batch].ysort) {
                _new_canvas_batch();
            ...
            switch (b->type) {
                ...
                case Item::TYPE_RECT: {
                    ...
                    if (r->texture != canvas_data.batches[canvas_data.current_batch].texture || canvas_data.batches[canvas_data.current_batch].type != Item::TYPE_RECT) {
                        _new_canvas_batch();
        ...
        // Y-sort the batches
        for (int i = 1; i <= canvas_data.current_batch; i++) {
        ...
        for (int i = 0; i <= canvas_data.current_batch; i++) {
            ...
            switch(canvas_data.batches[i].type) {
                case Item::TYPE_RECT: {
                    ...
                    glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_INT, nullptr, canvas_data.batches[i].instance_count);
...
void RenderingServerGL::_rebuild_viewport_texture(Viewport *p_viewport, Vector2i p_size) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, p_size.x, p_size.y, 0, GL_DEPTH_STENCIL, GL_UNSIGNED_INT_24_8, nullptr);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, p_viewport->texture_depth_id, 0);
...
void RenderingServerGL::mesh_free(RID p_rid) {
    if (mesh_owner.owns(p_rid)) {
        ...
        int buffer_size = (mesh->vertex_count * 2 * ((mesh->format & FORMAT_USE_2D_VERTICES) ? 2 : 3) + mesh->vertex_count * 2) * sizeof(float);
...
void RenderingServerGL::mesh_set_from_data(RID p_mesh, const MeshData &p_data) {
    ...
    utils->allocate_buffer(GL_ELEMENT_ARRAY_BUFFER, p_data.index_attribs.get_ptr_size(), p_data.index_attribs.ptr(), GL_STATIC_DRAW);
...
void RenderingServerGL::item_set_texture_rect(RID p_item, RID p_texture, const Vector2 &p_position, const Vector2 &p_size) {
    Item *i = canvas_item_owner.get_or_null(p_item);
    ERR_COND_NULL(i);
...
void RenderingServerGL::texture_set_from_data(RID p_texture, const u8 *p_data, int p_width, int p_height, TextureFormat p_format, TextureMask p_mask) {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, (p_mask == MASK_DEFAULT) ? GL_LINEAR_MIPMAP_LINEAR : mipmaps);
...
void RenderingServerGL::free(RID p_rid) {
    if (instance_owner.owns(p_rid)) {
        ...
        ERR_FAIL_COND_MSG(p_rid.is_valid() == true, "Could not free the given RID as its RID does not correspond to any current RID owners.");
    }
}
...
void RenderingServerGL::_enable_attributes(u32 p_start, u32 p_rate) {
    for (int i = 4; i < 9; i++) {
        ...
        glVertexAttribPointer(i, 4, GL_FLOAT, GL_FALSE, sizeof(CanvasInstanceData), (void *)(p_start + (i - 4) * 4 * sizeof(float)));
...
void RenderingServerGL::_new_canvas_batch() {
    ...
    cb.start = canvas_data.batches[canvas_data.current_batch].start + canvas_data.batches[canvas_data.current_batch].instance_count;
    cb.instance_count = 0;
    canvas_data.batches.push_back(cb);
...
RenderingServerGL::RenderingServerGL() {
    ...
    if (ext_list.find("GL_KHR_debug") >= 0) {
        ...
        print_verbose("Could not find the debug extensions string for OpenGL. OpenGL advanced debug output is disabled.");
    }

    // Setup lights

    // Directional lights
    scene_data.directional_lights = (SceneData::DirectionalLight *)Memory::vallocate(sizeof(SceneData::DirectionalLight) * 32);
    ...
    canvas_data.canvas_instance_data = (CanvasInstanceData *)Memory::vallocate(sizeof(CanvasInstanceData) * MAX_INSTANCE_DATA_COUNT);
