...
// Euler rotations derived from here:
// https://danceswithcode.net/engineeringnotes/rotations_in_3d/rotations_in_3d_part1.html

/**
 * @brief Structure that represents a 4x4 matrix, commonly used for transforms in 3D space. Our implementation uses a
 * row-major order, as opposed to OpenGL's column-major order, and thus any matrices used within OpenGL need to be
 * transposed prior to being taken into a vertex/fragment shader.
 */
struct Mat4 {
public:
	double data[16] = {0};
	...
	FORCE_INLINE void operator=(const Mat4 &p_other) {
		Memory::vmemcpy(data, (void *)p_other.data, sizeof(double) * 16);
	}
	...
	static FORCE_INLINE Mat4
	orthographic(double top, double bottom, double left, double right, double p_near, double p_far);
...
/**
 * @brief Multiplies two matrices together. Due to the nature of matrix transformations, the order one wants to
 * multiply two matrices by is right-to-left, such as XYZ rotations being actually computed in the order ZYX.
...
void Mat4::operator*=(const Mat4 &p_other) {
	Mat4 tmp = *this;
	data[0] =
		tmp.data[0] * p_other[0] + tmp.data[1] * p_other[4] + tmp.data[2] * p_other[8] + tmp.data[3] * p_other[12];
	data[1] =
		tmp.data[0] * p_other[1] + tmp.data[1] * p_other[5] + tmp.data[2] * p_other[9] + tmp.data[3] * p_other[13];
	data[2] =
		tmp.data[0] * p_other[2] + tmp.data[1] * p_other[6] + tmp.data[2] * p_other[10] + tmp.data[3] * p_other[14];
	data[3] =
		tmp.data[0] * p_other[3] + tmp.data[1] * p_other[7] + tmp.data[2] * p_other[11] + tmp.data[3] * p_other[15];
	data[4] =
		tmp.data[4] * p_other[0] + tmp.data[5] * p_other[4] + tmp.data[6] * p_other[8] + tmp.data[7] * p_other[12];
	data[5] =
		tmp.data[4] * p_other[1] + tmp.data[5] * p_other[5] + tmp.data[6] * p_other[9] + tmp.data[7] * p_other[13];
	data[6] =
		tmp.data[4] * p_other[2] + tmp.data[5] * p_other[6] + tmp.data[6] * p_other[10] + tmp.data[7] * p_other[14];
	data[7] =
		tmp.data[4] * p_other[3] + tmp.data[5] * p_other[7] + tmp.data[6] * p_other[11] + tmp.data[7] * p_other[15];
	data[8] =
		tmp.data[8] * p_other[0] + tmp.data[9] * p_other[4] + tmp.data[10] * p_other[8] + tmp.data[11] * p_other[12];
	data[9] =
		tmp.data[8] * p_other[1] + tmp.data[9] * p_other[5] + tmp.data[10] * p_other[9] + tmp.data[11] * p_other[13];
	data[10] =
		tmp.data[8] * p_other[2] + tmp.data[9] * p_other[6] + tmp.data[10] * p_other[10] + tmp.data[11] * p_other[14];
	data[11] =
		tmp.data[8] * p_other[3] + tmp.data[9] * p_other[7] + tmp.data[10] * p_other[11] + tmp.data[11] * p_other[15];
	data[12] =
		tmp.data[12] * p_other[0] + tmp.data[13] * p_other[4] + tmp.data[14] * p_other[8] + tmp.data[15] * p_other[12];
	data[13] =
		tmp.data[12] * p_other[1] + tmp.data[13] * p_other[5] + tmp.data[14] * p_other[9] + tmp.data[15] * p_other[13];
	data[14] = tmp.data[12] * p_other[2] + tmp.data[13] * p_other[6] + tmp.data[14] * p_other[10] +
			   tmp.data[15] * p_other[14];
	data[15] = tmp.data[12] * p_other[3] + tmp.data[13] * p_other[7] + tmp.data[14] * p_other[11] +
			   tmp.data[15] * p_other[15];
}
...
/**
 * @brief Returns an instance of the identity matrix, of which is a matrix that when multiplied against will produce no
 * change to the other matrix.
 */
Mat4 Mat4::identity() {
	Mat4 ret;
	ret.data[0] = 1.0;
...
/**
 * @brief Constructs a rotation matrix from three Euler angles. Order of rotation is YXZ or yaw-pitch-roll, as is the
...
/**
 * @brief Creates an orthographic project matrix, a projection by which two vertices will appear to be right next to
 * one another despite significant differences in position to the camera. The current implementation measure the height
 * and width from the top to the bottom of the screen for the y axis, and from the left to the right for the x axis.
...
/**
 * @brief Creates a perspective projection matrix, a projection by which two vertices will appear to have some depth
 * between one another, much like how one would see two objects appear in real life. The current implementation uses a
 * vertical FOV option in order to calculate what should be within the frustum or not.
...
/**
 * @brief Creates a "look-at" matrix, of which is used so that an object can look at a given point in space (its
 * target) from another point (its eye). The look-at matrix is commonly used within cameras so they can move freely
 * around a scene.
...
/**
 * @brief Constructs a model matrix from a given position, rotation and scale. Model matrices are a group of
 * translations one can make to a vertex to move it from its own local coordinates to the "global" space, or the space
 * of its parents. Should be used instead of traditional multiplication methods as it prevents unusual things occuring
 * with the translation step of the matrix
...
/**
 * @brief Obtains an orthonormalized version of the current matrix, which is a basis (only rotation and scale) that has
 * only rotation.
...
/**
 * @brief Obtains the rotational information from the current matrix as a quaternion. This method should be used in
 * favour of `get_quaternion_unsafe()` as that method does not orthonormalize the matrix first, which will give
 * incorrect results if the matrix has a scale factor not equal to 1.
...
/**
 * @brief Obtains the rotational quaternion from a matrix. This method assumes that the matrix it is used on has all of
 * its rows normalized.
 * @returns The rotational information of the matrix expressed as a quaternion
 */
...
/**
 * @brief Obtains the scale factor of the current matrix. Generally speaking, the scale should be equal to the length
 * of the first 3 components in each row combined into one `Vector3`.
 * @returns The scale factor of the current matrix
 */
Vector3 Mat4::get_scale() const {
	double s = determinant() < 0.0 ? -1 : 1;
	return Vector3(Vector3(data[0], data[1], data[2]).length(),
...
/**
 * @brief Obtains the translation factor of the current matrix. The translation factor can also be read as the position
 * of whatever object this matrix refers to, either in local or global space.
...
/**
 * @brief Obtains the Euler angles of a rotation matrix and returns them in a `Vector3`. Whilst there are many
 * different ways of finding the Euler angles given the various different combinations of angles one can have, we will
 * use the YXZ implementation for the moment, as alternative Euler angle support will require a re-do of much of the
 * code.
...
/**
 * @brief Obtains a version of the current matrix that has been rotated about a given axis by a given number of
 * radians.
 * @param p_axis The (normalized) axis by which to rotate about
 * @param p_angle The number of radians to rotate by
...
/**
 * @brief Rotates the current matrix locally (i.e. an object will rotate about the axes given based on its own position
 * in space rather than about the origin) about a given axis by a given number of radians.
 * @param p_axis The (normalized) axis by which to rotate about
 * @param p_angle The number of radians to rotate by
 */
...
/**
 * @brief Obtains a version of the current matrix that has been rotated locally about a given axis by a given number of
 * radians (for an explanation of the difference between local and regular rotations, see `rotate_local()`).
 * @param p_axis The (normalized) axis by which to rotate about
 * @param p_angle The number of radians to rotate by
...
/**
 * @brief Rotates the current matrix locally by a given quaternion (for an explanation of the difference between local
 * and regular rotations, see `rotate_local()`).
 * @param p_quaternion The quaternion to rotate the current matrix by
 */
...
/**
 * @brief Obtains a version of the current matrix that has been rotated locally by a given quaternion (for an
 * explanation of the difference between local and regular rotations, see `rotate_local()`).
 * @param p_quaternion The quaternion to rotate the current matrix by
 * @returns The current matrix rotated by the given quaternion
 */
