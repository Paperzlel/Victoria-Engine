...
void EditorCamera::_update(double p_delta) {
	...
	if (DisplayManager::get_singleton()->get_mouse_mode()) {
		...
		direction.x =
			Math::sin(-offset.x * Math::DEG_TO_RAD_MULTIPLIER) * Math::cos(offset.y * Math::DEG_TO_RAD_MULTIPLIER);
		direction.y = Math::sin(offset.y * Math::DEG_TO_RAD_MULTIPLIER);
		direction.z =
			Math::cos(-offset.x * Math::DEG_TO_RAD_MULTIPLIER) * Math::cos(offset.y * Math::DEG_TO_RAD_MULTIPLIER);
		direction.normalize();

		/**
		 * Since position data cannot be held within a matrix that is looking at a given point, we instead use this
		 * system, whereby one holds the actual position of the camera within the class itself, and the applied
		 * transform contains the (fake) position of the object.
		 */
