...
vec3 calculate_point_light(PointLight in_light, vec3 fragment, vec3 view_direction) {
    ...
    // Lambert term (https://stackoverflow.com/questions/20008089/specular-lighting-appears-on-both-eye-facing-and-rear-sides-of-object)
    ...
    if (dist < in_light.range) {
        attenuation = 1 / (1 + 0.08 * dist + 0.025 * (dist * dist));
    }
...
void main() {
    ...
    // Directional lights
    if (directional_lights_used > 0) {
        for (int i = 0; i < directional_lights_used; i++) {
            result += calculate_directional_light(directional_lights[i], view_dir);
        }
    }

    // Point lights
    if (point_lights_used > 0) {
        for (int i = 0; i < point_lights_used; i++) {
            result += calculate_point_light(point_lights[i], frag_pos, view_dir);
        }
    }

    if (spot_lights_used > 0) {
        for (int i = 0; i < spot_lights_used; i++) {
            result += calculate_spot_light(spot_lights[i], view_dir);
        }
    }

    if (directional_lights_used == 0 && point_lights_used == 0 && spot_lights_used == 0) {
        result = ambient;
    }
