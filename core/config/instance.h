...
/**
 * @brief Base class for all refcounted objects. Ref-counted, so that multiple bits of code can call to the same region of memory. 
 */
...
/**
 * @brief Template class for use with a ref-counted object, or any API-compliant class that extends Instance. Should be used over a raw Instance,
 * as it handles much of the referencing for the user.
 */
template <typename T>
class Ref {
    ...
    /**
     * @brief Increments the ref-count of a given pointer. If Init is true, then the ref-count is initialized, if not, then the ref-count is
     * incremented on the pointer.
...
public:
    ...
    /**
     * @brief Creates an instance of the given class, with a variadic number of arguments. Since Ref<T> objects don't instance the class on creation,
     * this needs to be called in order to prevent any memory errors as the class is otherwise null.
    ...
    /**
     * @brief Decrements the refcount and sets the reference for the object to null. If the refcount = 0 on the object, delete it.
     */
    void unref() {
        if (reference) {
