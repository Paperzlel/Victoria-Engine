...
void Light3D::set_colour(const Vector3 &p_colour) {
	...
	RM::get_singleton()->light_set_ambient(light, colour);
	RM::get_singleton()->light_set_diffuse(light, colour);
}
...
void Light3D::set_specular(const Vector3 &p_specular) {
	specular = p_specular;
	RM::get_singleton()->light_set_specular(light, specular);
}

Light3D::Light3D() {
	light = RM::get_singleton()->light_allocate();
	set_base(light);
...
Light3D::~Light3D() {
	RM::get_singleton()->light_free(light);
	set_base(RID());
}
...
void PointLight3D::set_range(float p_range) {
	range = p_range;
	RM::get_singleton()->light_set_range(light, range);
}

PointLight3D::PointLight3D() {
	RM::get_singleton()->light_set_type(light, RM::TYPE_POINT_LIGHT);
	set_range(10);
}
...
DirectionalLight3D::DirectionalLight3D() {
	RM::get_singleton()->light_set_type(light, RM::TYPE_DIRECTIONAL_LIGHT);
}
...
void SpotLight3D::set_inner_cone(float p_radius) {
	inner_cone = p_radius;
	RM::get_singleton()->light_set_radii(light, Math::cos(inner_cone), Math::cos(outer_cone));
}
...
SpotLight3D::SpotLight3D() {
	RM::get_singleton()->light_set_type(light, RM::TYPE_SPOT_LIGHT);
	set_inner_cone(10 * Math::DEG_TO_RAD_MULTIPLIER);
	set_outer_cone(12.5 * Math::DEG_TO_RAD_MULTIPLIER);
	RM::get_singleton()->light_set_range(light, 10);
}
