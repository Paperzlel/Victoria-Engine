...
/**
 * @brief Simplifies the vertex data by trimming out the previous duplicate vertices, and obtains the index array for
 * the mesh.
 */
...
Ref<Resource> ResourceFormatImporterOBJ::_import(const String &p_path, int p_argc, Variant *p_args) {
	...
	while (fs->get_position() < fs->get_length()) {
		...
		// WARNING: We automatically assume that the user has vertex/vertex coords/vertex normals for their face
		...
		if (sline[0] == "f") {
			for (int i = 0; i < 3; i++) {
				...
				if (substr.size() < 3) {
					ERR_WARN(vformat("Given vertex indicies (%s) were not in the format v/vt/vn.",
									 sline[i + 1].get_data()));
					continue;
				}
